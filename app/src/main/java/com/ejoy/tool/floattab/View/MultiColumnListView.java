...
import com.ejoy.tool.floattab.lib.ParcelableSparseIntArray;
import com.ejoy.tool.floattab.lib.internal.PLA_AbsListView;
import com.ejoy.tool.floattab.lib.internal.PLA_ListView;
...
public class MultiColumnListView extends PLA_ListView {
	...
	private HorizontalListView mSupLayout = AppConstant.MY_INDICATOR;
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (deltaY > 0) {
					...
					if (getFirstVisiblePosition() == 0 && getChildAt(0).getTop() == 0) {
						AppConstant.RET = true;
					} else {
						AppConstant.RET = false;
					}
				} else if (deltaY < 0) { //上啦过程
                    if (mSupLayout.getVisibility() ==GONE){ //特殊滑动状态
						AppConstant.RET = true; //转交事件
                      return false;
                    }else {
						AppConstant.RET =false;
					}
