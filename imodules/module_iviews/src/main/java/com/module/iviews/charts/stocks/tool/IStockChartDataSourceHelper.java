...
/**
 * @ClassName:  IStockChartDataSourceHelper
 * @Author: maiduoduo
 * @BLOG: https://blog.csdn.net/Maiduoudo
 * @Date: 2020/03/14
 * @des: 行情图数据辅助类
 */
public class IStockChartDataSourceHelper {
  ...
  private final IStockTechParamsHelper mIStockTechParamsHelper;
  ...
  public IStockChartDataSourceHelper(IChartDataCountListener<List<KLineToDrawItem>> listener) {
    this.mReadyListener = listener;
    mIStockTechParamsHelper = new IStockTechParamsHelper();
  }
  ...
  public void initKDrawData(List<KLineItem> klineList,
      IKMasterChartView kLineChartView,
      IKSubChartView volumeView, IKSubChartView macdView) {
    ...
    mIStockSubChartData = new IStockSubChartData();
    ...
    // 计算技术指标
    mIStockTechParamsHelper.caculateTechParams(klineList, IStockTechParamType.BOLL);
    mIStockTechParamsHelper.caculateTechParams(klineList, IStockTechParamType.MACD);
  ...
  public void initKMoveDrawData(float distance, SourceType sourceType) {
    ...
    // 计算蜡烛线价格最大最小值，成交量最大值
    IStockExtremeValue extremeValue = countMaxMinValue();
    ...
    for (int i = startIndex, k = 0; i < endIndex; i++, k++) {
      ...
      if (i - 1 >= 0 && i + 1 < endIndex) {
        int currentMonth = IDateUtils.getMonth(kLineItem.day);
        KLineItem preItem = mKList.get(i - 1);
        int preMonth = IDateUtils.getMonth(preItem.day);
      ...
      // 计算成交量
      if (Math.abs(maxVolume) > IStockDataUtils.EPSILON) {
    ...
    if (mReadyListener != null) {
      ...
      mReadyListener.onReady(resultList, extremeValue, mIStockSubChartData);
    }
  }
  ...
  private void caculateBollPath(float diffBoll, RectF contentRect, int i, int k,
    ...
    List<IStockTechItem> listParams = mIStockTechParamsHelper.listParams;
    IStockTechItem techItem = listParams.get(i);
    drawItem.techItem = techItem;
    float scaleY = (maxBoll - techItem.upper) / diffBoll;
    PointF point = getPoint(contentRect, k, scaleY);
    if (i == startIndex) {
      mIStockSubChartData.bollPaths[0].moveTo(point.x, point.y);
    } else {
      mIStockSubChartData.bollPaths[0].lineTo(point.x, point.y);
    }

    scaleY = (maxBoll - techItem.lower) / diffBoll;
    point = getPoint(contentRect, k, scaleY);
    if (i == startIndex) {
      mIStockSubChartData.bollPaths[1].moveTo(point.x, point.y);
    } else {
      mIStockSubChartData.bollPaths[1].lineTo(point.x, point.y);
    }
    ...
    point = getPoint(contentRect, k, scaleY);
    if (i == startIndex) {
      mIStockSubChartData.bollPaths[2].moveTo(point.x, point.y);
    } else {
      mIStockSubChartData.bollPaths[2].lineTo(point.x, point.y);
    }
  }
  ...
  private void caculateMacdPath(float diffMacd, int index, int k, boolean isFall) {
    List<IStockTechItem> listParams = mIStockTechParamsHelper.listParams;
    IStockTechItem techItem = listParams.get(index);
    RectF contentRect = mMacdView.getViewPortHandler().mContentRect;
    float scaleY = (maxMacd - techItem.dif) / diffMacd;
    PointF point = getPoint(contentRect, k, scaleY);
    if (index == startIndex) {
      mIStockSubChartData.macdPaths[0].moveTo(point.x, point.y);
    } else {
      mIStockSubChartData.macdPaths[0].lineTo(point.x, point.y);
    }

    scaleY = (maxMacd - techItem.dea) / diffMacd;
    point = getPoint(contentRect, k, scaleY);
    if (index == startIndex) {
      mIStockSubChartData.macdPaths[1].moveTo(point.x, point.y);
    } else {
      mIStockSubChartData.macdPaths[1].lineTo(point.x, point.y);
    }
    scaleY = techItem.macd / diffMacd;
    RectF rect = getRect(contentRect, k, scaleY);
    IStockLineRectItem IStockLineRectItem = new IStockLineRectItem();
    IStockLineRectItem.rect = rect;
    IStockLineRectItem.isFall = techItem.macd < IStockDataUtils.EPSILON;
    mIStockSubChartData.macdRects.add(IStockLineRectItem);
  }
  ...
  private void countStartEndPos(float distance,
    ...
    if (sourceType == SourceType.SCALE) {
      ...
      // 根据偏移距离计算偏移几天
      int offCount = (int) ((distance * IStockChartDataSourceHelper.K_D_COLUMNS)
   ...
   * 计算蜡烛线价格最大最小值，成交量最大值
   */
  private IStockExtremeValue countMaxMinValue() {
    List<IStockTechItem> listParams = mIStockTechParamsHelper.listParams;
    for (int i = startIndex; i < endIndex; i++) {
      ...
      // 计算MACD最大值最小值
      IStockTechItem techItem = listParams.get(i);
      IStockTechParamsHelper.Limit limit = mIStockTechParamsHelper.getLimitValue(techItem, IStockTechParamType.MACD);
      ...
      // 计算BOLL最大最小值
      IStockTechParamsHelper.Limit limitValue =
          mIStockTechParamsHelper.getLimitValue(techItem, IStockTechParamType.BOLL);
    ...
    IStockExtremeValue extremeValue = new IStockExtremeValue();
