 ...
 * Date:   2019/12/31
 * Des:    TODO:提供一个浮层，支持自定义浮层的内容，支持在指定 {@link View} 的任一方向旁边展示该浮层，支持自定义浮层出现/消失的动画。
 */
public class EUIPopup extends EBasePopup {
    ...
    public EUIPopup(Context context) {
        this(context, DIRECTION_NONE);
        this.mContext = context;
    }


    public EUIPopup(Context context, int preferredDirection) {
        super(context);
     ...
     * 设置根据计算得到的位置后的偏移值
     */
    public EUIPopup setPositionOffsetX(int offsetX) {
        mOffsetX = offsetX;
        return this;
    }
     ...
     */
    public EUIPopup setPositionOffsetYWhenTop(int offsetYWhenTop) {
     ...
     */
    public EUIPopup setPositionOffsetYWhenBottom(int offsetYWhenBottom) {
     ...
     * @return
     */
    public EUIPopup setPreferredDirection(int preferredDirection) {
        mPreferredDirection = preferredDirection;
        return this;
    }
    ...
    private void initAnimationStyle(int screenWidth, int requestedX) {
        ...
        switch (mAnimStyle) {
            case ANIM_GROW_FROM_LEFT:
                getPopupWindow().setAnimationStyle(onTop ? R.style.EUI_Animation_PopUpMenu_Left : R.style.EUI_Animation_PopDownMenu_Left);
                break;

            case ANIM_GROW_FROM_RIGHT:
                getPopupWindow().setAnimationStyle(onTop ? R.style.EUI_Animation_PopUpMenu_Right : R.style.EUI_Animation_PopDownMenu_Right);
                break;

            case ANIM_GROW_FROM_CENTER:
                getPopupWindow().setAnimationStyle(onTop ? R.style.EUI_Animation_PopUpMenu_Center : R.style.EUI_Animation_PopDownMenu_Center);
                break;
            case ANIM_AUTO:
    ...
    public void setContentView(View root) {
        FrameLayout container = (FrameLayout) LayoutInflater.from(getContext()).inflate(R.layout.ejoy_layout_popup, null, false);
    ...
    public void showDown(View v) {
        setPreferredDirection(EUIPopup.DIRECTION_BOTTOM);
        show(v);
    }
    ...
    public void showUp(View v) {
        setPreferredDirection(EUIPopup.DIRECTION_TOP);
        show(v);
    }
    ...
    protected int getPopupWidth() {
        int width;
        switch (EDeviceUtils.getScreenType(mContext)) {
            case CommonConstant.ScreenType.BIG_TABLET:
                width = EDensityUtils.getDimensionPixelSize(mContext,R.dimen.ejoy_popup_width_tablet_big);
                break;
            case CommonConstant.ScreenType.SMALL_TABLET:
                width = EDensityUtils.getDimensionPixelSize(mContext,R.dimen.ejoy_popup_width_tablet_small);
                break;
            case CommonConstant.ScreenType.PHONE:
                width = EDensityUtils.getDimensionPixelSize(mContext,R.dimen.ejoy_popup_width_phone);
                break;
            default:
                width = EDensityUtils.getDimensionPixelSize(mContext,R.dimen.ejoy_popup_width_tablet_small);
                break;
