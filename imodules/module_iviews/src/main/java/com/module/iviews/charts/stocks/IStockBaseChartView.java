...
import com.module.iviews.charts.stocks.listener.ITouchResponseListener;
...
/**
 * @ClassName:  IStockBaseChartView
 * @Author: maiduoduo
 * @BLOG: https://blog.csdn.net/Maiduoudo
 * @Date: 2021/2/20
 * @des: 具体绘制View的基类：（1）采用双缓冲绘制机制 （2）支持在子线程当中进行绘制
 */
public abstract class IStockBaseChartView extends View implements ITouchResponseListener {
   ...
   * 行情图尺寸等辅助方法
   */
  protected IStockViewPortHandler mViewPortHandler;
  ...
  public IStockBaseChartView(Context context) {
    this(context, null);
  }
  ...
  public IStockBaseChartView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    // 行情图尺寸等辅助方法
    mViewPortHandler = new IStockViewPortHandler();

    initHandler();

    EPaintUtils.init(context);
  }
  ...
  private static final class ChartHandler extends Handler {
    ...
    private final WeakReference<IStockBaseChartView> mView;

    public ChartHandler(IStockBaseChartView view) {
    ...
    public void handleMessage(@NonNull Message msg) {
      ...
      if (mView != null) {
        IStockBaseChartView chartView = mView.get();
        if (chartView != null) {
  ...
  class DoubleBuffering implements Runnable {
    ...
    private final WeakReference<IStockBaseChartView> mChartView;

    public DoubleBuffering(IStockBaseChartView view) {
    ...
    public synchronized void run() {
      if (mChartView != null) {
        IStockBaseChartView baseChartView = mChartView.get();
