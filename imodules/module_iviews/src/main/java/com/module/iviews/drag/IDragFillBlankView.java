package com.module.iviews.drag;
 ...
 * 拖拽填空题
 */
public class IDragFillBlankView extends RelativeLayout implements View.OnDragListener,
    ...
    // 初始答案范围集合
    private List<IAnswerRange> originAnswerRangeList;
    // 填空题内容
    private SpannableStringBuilder content;
    // 选项列表
    private List<String> optionList;
    // 答案范围集合
    private List<IAnswerRange> answerRangeList;
    // 答案集合
    ...
    public IDragFillBlankView(Context context) {
        this(context, null);
    }

    public IDragFillBlankView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public IDragFillBlankView(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     */
    public void setData(String originContent, List<String> optionList, List<IAnswerRange> answerRangeList) {
        ...
        // 设置下划线颜色
        for (IAnswerRange range : this.answerRangeList) {
        ...
        for (int i = 0; i < this.answerRangeList.size(); i++) {
            IAnswerRange range = this.answerRangeList.get(i);
        ...
        // 填空处设置触摸事件
        tvContent.setMovementMethod(new ITouchLinkMovementMethod());
    ...
    public boolean onDrag(View v, DragEvent event) {
        ...
        switch (action) {
            ...
            case DragEvent.ACTION_DROP: // 放开被拖拽View
                ...
                for (int i = 0; i < answerRangeList.size(); i++) {
                    IAnswerRange range = answerRangeList.get(i);
    ...
    private void fillAnswer(String answer, int position) {
        ...
        // 替换答案
        IAnswerRange range = answerRangeList.get(position);
        content.replace(range.start, range.end, answer);

        // 更新当前的答案范围
        IAnswerRange currentRange = new IAnswerRange(range.start, range.start + answer.length());
        ...
        for (int i = 0; i < answerRangeList.size(); i++) {
            if (i > position) {
                // 获取下一个答案原来的范围
                IAnswerRange oldNextRange = answerRangeList.get(i);
                int oldNextAmount = oldNextRange.end - oldNextRange.start;
                // 计算新旧答案字数的差值
                int difference = currentRange.end - range.end;

                // 更新下一个答案的范围
                IAnswerRange nextRange = new IAnswerRange(oldNextRange.start + difference,
