...
/**
 * @ClassName:  IConsecutiveScrollerLayout
 * @Author: maiduoduo
 * @BLOG: https://blog.csdn.net/Maiduoudo
 * @Date: 2020/9/18
 * @des: 流畅滑动view,支持嵌套多个子view，支持吸顶、下沉
 */
public class IConsecutiveScrollerLayout extends ViewGroup implements ScrollingView, NestedScrollingParent2, NestedScrollingChild2 {
    ...
    public IConsecutiveScrollerLayout(Context context) {
        this(context, null);
    }

    public IConsecutiveScrollerLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public IConsecutiveScrollerLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray a = null;
        try {
            a = context.obtainStyledAttributes(attrs, R.styleable.IConsecutiveScrollerLayout);
            isPermanent = a.getBoolean(R.styleable.IConsecutiveScrollerLayout_isPermanent, false);
        } finally {
            if (a != null) {
                a.recycle();
            }
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        ...
        if (params instanceof LayoutParams) {
            ILayoutParamsUtils.invalidTopAndBottomMargin((LayoutParams) params);
        }

        super.addView(child, index, params);

        // 去掉子View的滚动条。选择在这里做这个操作，而不是在onFinishInflate方法中完成，是为了兼顾用代码add子View的情况
        if (IScrollUtils.isConsecutiveScrollerChild(child)) {
    ...
    protected void measureChildWithMargins(View child, int parentWidthMeasureSpec, int widthUsed, int parentHeightMeasureSpec, int heightUsed) {
        ILayoutParamsUtils.invalidTopAndBottomMargin((LayoutParams) child.getLayoutParams());
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mDownLocation[0] = IScrollUtils.getRawX(this, ev, actionIndex);
                mDownLocation[1] = IScrollUtils.getRawY(this, ev, actionIndex);
                isTouchNotTriggerScrollStick = IScrollUtils.isTouchNotTriggerScrollStick(this, mDownLocation[0], mDownLocation[1]);
                break;
            ...
            case MotionEvent.ACTION_POINTER_UP:
                if (mActivePointerId == ev.getPointerId(actionIndex)) { // 如果松开的是活动手指, 让还停留在屏幕上的最后一根手指作为活动手指
                    ...
                    mDownLocation[0] = IScrollUtils.getRawX(this, ev, newPointerIndex);
                    mDownLocation[1] = IScrollUtils.getRawY(this, ev, newPointerIndex);
                    isTouchNotTriggerScrollStick = IScrollUtils.isTouchNotTriggerScrollStick(this, mDownLocation[0], mDownLocation[1]);
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mAdjustVelocityTracker != null) {
                    ...
                    int touchX = IScrollUtils.getRawX(this, ev, actionIndex);
                    int touchY = IScrollUtils.getRawY(this, ev, actionIndex);
                    boolean canScrollVerticallyChild = IScrollUtils.canScrollVertically(getTouchTarget(touchX, touchY));
                    if (SCROLL_ORIENTATION != SCROLL_VERTICAL && canScrollVerticallyChild
                            && Math.abs(yVelocity) >= mMinimumVelocity
                            && !IScrollUtils.isHorizontalScroll(this, touchX, touchY)) {
                        //如果当前是横向滑动，但是触摸的控件可以垂直滑动，并且产生垂直滑动的fling事件，
                        // 为了不让这个控件垂直fling，把事件设置为MotionEvent.ACTION_CANCEL。
                        ev.setAction(MotionEvent.ACTION_CANCEL);
                    }

                    if (SCROLL_ORIENTATION == SCROLL_NONE && !IScrollUtils.isConsecutiveScrollParent(this)
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (IScrollUtils.isConsecutiveScrollParent(this) // 如果父级容器设置isConsecutive：true，则自己不消费滑动
...
//                }
        ...
        do {
            ...
            if (!isScrollBottom()) {
                ...
                if (firstVisibleView != null) {
                    awakenScrollBars();
                    int bottomOffset = IScrollUtils.getScrollBottomOffset(firstVisibleView);
                    if (bottomOffset > 0) {
        ...
        do {
            ...
            if (!isScrollTop()) {
                ...
                if (lastVisibleView != null) {
                    awakenScrollBars();
                    int childScrollOffset = IScrollUtils.getScrollTopOffset(lastVisibleView);
    ...
    private void scrollChild(View child, int y) {
        View scrolledView = IScrollUtils.getScrolledView(child);
        if (scrolledView instanceof AbsListView) {
            ...
            if (scrolledView instanceof RecyclerView) {
                isInterceptRequestLayout = IScrollUtils.startInterceptRequestLayout((RecyclerView) scrolledView);
            }
            ...
            if (isInterceptRequestLayout) {
                ...
                view.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        IScrollUtils.stopInterceptRequestLayout(view);
                    }
                }, 0);
            }
        }
    ...
    private void checkTargetsScroll(boolean isLayoutChange, boolean isForce) {
        ...
        if (isLayoutChange) {
            int bottomOffset = IScrollUtils.getScrollBottomOffset(target);
    ...
    private void scrollChildContentToTop(View target) {
        ...
        do {
            scrollY = 0;
            int offset = IScrollUtils.getScrollTopOffset(target);
            if (offset < 0) {
                int childOldScrollY = IScrollUtils.computeVerticalScrollOffset(target);
                scrollChild(target, offset);
                scrollY = childOldScrollY - IScrollUtils.computeVerticalScrollOffset(target);
            }
        } while (scrollY != 0);
    }
    ...
    private void scrollChildContentToBottom(View target) {
        ...
        do {
            scrollY = 0;
            int offset = IScrollUtils.getScrollBottomOffset(target);
            if (offset > 0) {
                int childOldScrollY = IScrollUtils.computeVerticalScrollOffset(target);
                scrollChild(target, offset);
                scrollY = childOldScrollY - IScrollUtils.computeVerticalScrollOffset(target);
...
//        // 导致ConsecutiveScrollerLayout追踪布局的滑动出现偏差
    ...
    public boolean isScrollTop() {
        ...
        if (size > 0) {
            ...
            boolean isScrollTop = getScrollY() <= 0 && !IScrollUtils.canScrollVertically(child, -1);

            if (isScrollTop) {
                for (int i = 0; i < size; i++) {
                    View view = children.get(i);
                    if (view instanceof IConsecutiveViewPager) {
                        IConsecutiveViewPager viewPager = (IConsecutiveViewPager) view;
                        if (viewPager.getAdjustHeight() > 0 && IScrollUtils.isConsecutiveScrollerChild(viewPager)
                                && IScrollUtils.canScrollVertically(viewPager, -1)) {
                            return false;
                        }
                    }
                }
    ...
    public boolean isScrollBottom() {
        ...
        if (children.size() > 0) {
            ...
            return getScrollY() >= mScrollRange && !IScrollUtils.canScrollVertically(child, 1);
        }
        return true;
    }
    ...
    public int computeVerticalScrollRange() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!IScrollUtils.isConsecutiveScrollerChild(child)) {
                ...
                if (IScrollUtils.canScrollVertically(child)) {
                    View view = IScrollUtils.getScrolledView(child);
                    range += IScrollUtils.computeVerticalScrollRange(view) + view.getPaddingTop() + view.getPaddingBottom();
                } else {
                    range += child.getHeight();
                }
            }
    ...
    public int computeVerticalScrollOffset() {
        ...
        for (int i = 0; i < count; i++) {
            View child = children.get(i);
            if (IScrollUtils.isConsecutiveScrollerChild(child)) {
                scrollOffset += IScrollUtils.computeVerticalScrollOffset(child);
            }
        }
    ...
    private View getTouchTarget(int touchX, int touchY) {
        ...
        for (View touchableView : touchableViews) {
            if (IScrollUtils.isTouchPointInView(touchableView, touchX, touchY)) {
    ...
    private boolean isIntercept(MotionEvent ev) {
        ...
        return isIntercept(IScrollUtils.getRawX(this, ev, pointerIndex),
                IScrollUtils.getRawY(this, ev, pointerIndex));
    }
    ...
    private boolean isIntercept(int touchX, int touchY) {
        ...
        if (target != null) {
            return IScrollUtils.isConsecutiveScrollerChild(target);
        }
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            TypedArray a = null;
            try {
                a = c.obtainStyledAttributes(attrs, R.styleable.IConsecutiveScrollerLayout_Layout);

                isConsecutive = a.getBoolean(R.styleable.IConsecutiveScrollerLayout_Layout_layout_isConsecutive, true);
                isNestedScroll = a.getBoolean(R.styleable.IConsecutiveScrollerLayout_Layout_layout_isNestedScroll, true);
                isSticky = a.getBoolean(R.styleable.IConsecutiveScrollerLayout_Layout_layout_isSticky, false);
                isTriggerScroll = a.getBoolean(R.styleable.IConsecutiveScrollerLayout_Layout_layout_isTriggerScroll, false);
                isSink = a.getBoolean(R.styleable.IConsecutiveScrollerLayout_Layout_layout_isSink, false);
                int type = a.getInt(R.styleable.IConsecutiveScrollerLayout_Layout_layout_align, 1);
    ...
    private int getViewsScrollOffset(int index) {
        ...
        for (int i = index; i < count; i++) {
            View child = children.get(i);
            if (child.getVisibility() != GONE && IScrollUtils.isConsecutiveScrollerChild(child)) {
                offset += IScrollUtils.computeVerticalScrollOffset(child);
            }
        }
        return offset;
    }
