...
public class RefreshLoadMoreUtil {
    ...
    public static boolean isContentToItem(RecyclerView recyclerView,
        ...
        if (targetIndex < 0 || targetIndex >= adapItemCount) {
            if (ELog.isEnabled()) ELog.w("Invalid targetIndex = %d", targetIndex);
            return false;
        } else {
            if (ELog.isEnabled()) ELog.v("targetIndex = %d", targetIndex);
        }
        int[] poss = getRecyclerViewFirstCompleteVisiblePos(recyclerView, true, isFirst);
        boolean match = false;
        for (int p : poss) {
            if (ELog.isEnabled()) ELog.v("complete visible position = %d", p);
            if (targetIndex == p) {
                match = true;
                break;
                ...
                if (type == Type.TOP && 0 == targetIndex) {
                    RecyclerView.ViewHolder viewHolder = recyclerView.findViewHolderForAdapterPosition(
                            targetIndex);
                    if (null == viewHolder) {
                        if (ELog.isEnabled()) ELog.v("null == viewHolder");
                        match = false;
                    } else {
                        ...
                        if (ELog.isEnabled())
                            ELog.v("first item view top = %d, topMargin = %d", top, topMargin);
