...
import com.iduo.filterlib.StatusBarHelper;
import com.module.ires.bean.utils.EFindViewUtil;
import com.module.ires.bean.utils.ERomUtil;
...
//            StatusBarUtil.fitsNotchScreen(window, true);
    ...
    private void addNavigationBar(Window window) {
        ...
        if (mLinearLayout == null) {
            mLinearLayout = EFindViewUtil.getTargetView(window.getDecorView(), LinearLayout.class);
        }
        //避免重复添加
        if (mLinearLayout != null && mPlusNavigationViewEnable) {
            //创建假的NavigationView包裹ViewGroup用于设置背景与mContentView一致
            mLayoutNavigation = EFindViewUtil.getTargetView(window.getDecorView(), R.id.fake_navigation_layout);
    ...
    private void setNavigationView(Window window) {
        if (mLayoutNavigation != null) {
            ...
            navigationHeight += angle == Surface.ROTATION_270 && navigationHeight > 0 && ERomUtil.isEMUI() ? StatusBarHelper.getStatusbarHeight(window.getDecorView().getContext()) : 0;
            ...
            if (mPlusDecorViewEnable) {
                ...
                if (!ERomUtil.isEMUI() && !isNavigationAtBottom && angle == Surface.ROTATION_270) {
            ...
            if (!ERomUtil.isEMUI() && !isNavigationAtBottom && angle == Surface.ROTATION_270 && index != 0) {
