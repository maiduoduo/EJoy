...
//    private final static boolean ONLY_ID_CARD = false;
    ...
    private void init(Context context) {
        ...
        // 默认三种图标
        this.delDrawable = context.getDrawable(R.drawable.icon_safe_del);
        this.lowDrawable = context.getDrawable(R.drawable.icon_safe_capital_default);
        this.upDrawable = context.getDrawable(R.drawable.icon_safe_capital_selected);
        this.upDrawableLock = context.getDrawable(R.drawable.icon_safe_capital_selected_lock);
...
//    }
    ...
    private void drawSpecialKey(Canvas canvas, Keyboard.Key key) {
        ...
        if (key.codes[0] == -5) {
            drawKeyBackground(R.drawable.safe_keyboard_change, canvas, key);
        ...
        try {
            ...
            if (key.label != null) {
                ...
                if (label.length() > 1 && key.codes.length < 2) {
                    int labelTextSize = 0;
                    try {
                        field = KeyboardView.class.getDeclaredField(getContext().getString(R.string.safe_keyboard_mLabelTextSize));
