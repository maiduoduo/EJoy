package com.module.iviews.slidebar;
...
import com.module.ires.bean.utils.EColorUtil;
...
/**
 * @ClassName:  IWaveSideBarView
 * @Author: maiduoduo
 * @BLOG: https://blog.csdn.net/Maiduoudo
 * @Date: 2020/9/17
 * @des:  字母导航的sidebarview。类似美团城市选择器，可用于recyclerview的首写单词快选
 */
public class IWaveSideBarView extends View {
    ...
    public IWaveSideBarView(Context context) {
        this(context , null);
    }

    public IWaveSideBarView(Context context, AttributeSet attrs) {
        this(context, attrs , 0);
    }

    public IWaveSideBarView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    private void init(Context context, AttributeSet attrs) {
        mLetters = Arrays.asList(context.getResources().getStringArray(R.array.iwaveSideBarLetters));

        mTextColor = Color.parseColor("#969696");
//        mWaveColor = EColorUtil.getColorPrimary(context);
        mWaveColor = EColorUtil.getColorPrimary(context);
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.IWaveSideBarView);
            mTextColor = a.getColor(R.styleable.IWaveSideBarView_sidebarTextColor, mTextColor);
            mTextColorChoose = a.getColor(R.styleable.IWaveSideBarView_sidebarChooseTextColor, mTextColorChoose);
            mTextSize = a.getFloat(R.styleable.IWaveSideBarView_sidebarTextSize, mTextSize);
            mWaveTextSize = a.getFloat(R.styleable.IWaveSideBarView_sidebarWaveTextSize, mWaveTextSize);
            mWaveColor = a.getColor(R.styleable.IWaveSideBarView_sidebarBackgroundColor, mWaveColor);
            mRadius = a.getInt(R.styleable.IWaveSideBarView_sidebarRadius, context.getResources().getDimensionPixelSize(R.dimen.radius_sidebar));
            mBallRadius = a.getInt(R.styleable.IWaveSideBarView_sidebarBallRadius, context.getResources().getDimensionPixelSize(R.dimen.ball_radius_sidebar));
            a.recycle();
        }
